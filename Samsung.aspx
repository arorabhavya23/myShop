<%@ Page Title="" Language="C#" MasterPageFile="~/DataMiningMasterPage.master" AutoEventWireup="true" CodeFile="Samsung.aspx.cs" Inherits="Samsung" %>
...
<asp:ImageButton ID="ImageButton1" runat="server" Height="27px" commandargument='<%# Eval("Prodid") %>' ImageUrl='~/css/images/addtocart.gif' Width="73px" OnClick="ImageButton1_Click" />
        ...
        <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:ecomdatabaseConnectionString %>" SelectCommand="SELECT * FROM [NewProduct] WHERE ([SubCatgId] = '5')"></asp:SqlDataSource>
        <asp:SqlDataSource ID="SqlDataSource3" runat="server" ConnectionString="<%$ ConnectionStrings:ecomdatabaseConnectionString %>" SelectCommand="SELECT [ProductName], [MRP], [OurPrice], [Type_of_Product], [image] FROM [MainProduct] where [Brand]='Samsung';"></asp:SqlDataSource>
