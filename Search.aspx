<%@ Page Title="" Language="C#" MasterPageFile="~/DataMiningMasterPage.master" AutoEventWireup="true" CodeFile="Search.aspx.cs" Inherits="Search" %>
...
<asp:ImageButton ID="ImageButton1" runat="server" Height="27px" commandargument='<%# Eval("Prodid") %>' ImageUrl='~/css/images/addtocart.gif' Width="73px" OnClick="ImageButton1_Click" />
        ...
        <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:ecomdatabaseConnectionString %>" SelectCommand="SELECT * FROM [NewProduct] WHERE ([ProductName] = @ProductName)">
            <SelectParameters>
                <asp:QueryStringParameter Name="ProductName" QueryStringField="pid" Type="String" />
            </SelectParameters>
        </asp:SqlDataSource>
        <asp:SqlDataSource ID="SqlDataSource7" runat="server" ConnectionString="<%$ ConnectionStrings:ecomdatabaseConnectionString %>" SelectCommand="SELECT * FROM [NewProduct] WHERE (Pr = '6')">
        </asp:SqlDataSource>
